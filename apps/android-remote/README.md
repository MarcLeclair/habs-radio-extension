...
Android phone remote for controlling the Habs Radio Sync TV app on the same local network. It uses the TV app's local HTTP control API for playback, delay, volume, and station changes.
...
1. Start Habs Radio Sync TV on the Android TV device.
2. Connect the phone to the same Wi-Fi network.
3. Open Habs Radio Remote and connect to the TV.
